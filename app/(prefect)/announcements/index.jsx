// =====================================================================
// app/(prefect)/(tabs)/announcements.jsx
// ---------------------------------------------------------------------
// Ruta "/announcements" del tab "Avisos" del prefecto.
//
...
// Tap en card → navega a detalle (/(prefect)/announcements/:id).
// =====================================================================
...
// Servicio del prefecto (para grupos y alumnos del modal).
import {
  getGroups,
  getGroupStudents,
...
// =====================================================================
export default function PrefectAnnouncementsScreen() {
  ...
  const mapPrefectGroup = useCallback((g) => ({
    id: g._id,
  ...
  const {
    ...
    mapGroupFn: mapPrefectGroup,
    fetchStudentsFn: fetchPrefectStudents,
    onPublished: refetch,
  });
  ...
  const renderAnnouncement = useCallback(({ item }) => {
    ...
    return (
      <Pressable
        onPress={() => router.push(`/(prefect)/announcements/${item._id}`)}
  ...
  return (
    <View className="flex-1 bg-[#F8FAFC]">
      ...
      {/* SCHOOL INFO CARD — modo compuesto (escuela + prefecto + fecha). */}
      <SchoolInfoCard
        school={school}
        isLoading={!school}
        className="mx-4 mt-4"
