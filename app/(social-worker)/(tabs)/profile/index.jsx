// =====================================================================
// app/(social-worker)/(tabs)/profile.jsx
// ---------------------------------------------------------------------
// Ruta "/profile" del tab "Perfil" del trabajador social.
// Perfil personal con información del trabajador social, seguridad y cerrar sesión.
// =====================================================================
...
export default function SocialWorkerProfileScreen() {
  ...
  // Datos del dashboard: school info para SchoolInfoCard.
  ...
  const socialWorkerName = dashboardData?.socialWorker?.name || user?.name || 'Trabajador Social';
  const socialWorkerLastName = dashboardData?.socialWorker?.last_name || user?.last_name || '';
  const fullName = `${socialWorkerLastName} ${socialWorkerName}`.trim() || socialWorkerName;
  const roleLabel = 'Trabajo Social';
    ...
    <View className="flex-1 bg-slate-50">
      ...
      >
        ...
        >
          ...
          >
            <View className="flex-row items-center py-1.5">
              ...
              <Text className="ml-3" style={{ fontSize: 13, fontWeight: '600', color: '#334155' }}>
                Tel: {dashboardData?.socialWorker?.phoneNumber || user?.phoneNumber || '—'}
              </Text>
            </View>
            ...
            <View className="flex-row items-center py-1.5">
              ...
              <Text className="ml-3" style={{ fontSize: 13, fontWeight: '600', color: '#334155' }}>
                {dashboardData?.socialWorker?.email || user?.email || '—'}
              </Text>
            </View>
          </View>
        ...
        <Text
          ...
          style={{ fontSize: 11, fontWeight: '700', color: '#64748B', textTransform: 'uppercase', letterSpacing: 0.5 }}
        >
          Seguridad y Cuenta
        </Text>
        <View
        ...
        >
          <ProfileRow
            ...
            onPress={() => router.push('/(social-worker)/(tabs)/profile/change-password')}
          />
        </View>
        ...
        {/* VERSIÓN */}
        <Text className="text-center mt-5" style={{ fontSize: 12, fontWeight: '500', color: '#94A3B8' }}>
          Versión: 1.0.0
        </Text>
        ...
        >
          <View className="flex-row items-center justify-center py-3.5">
            ...
            <Text className="ml-2" style={{ fontSize: 15, fontWeight: '700', color: '#DC2626' }}>
              Cerrar Sesión
            </Text>
          </View>
        </Pressable>
...
const ProfileRow = ({ icon: Icon, iconBg, iconColor, label, trailing, onPress }) => (
  ...
  >
    <View
      ...
      style={{ width: 36, height: 36, borderRadius: 10, backgroundColor: iconBg }}
    >
      <Icon size={18} color={iconColor} strokeWidth={2.25} />
    </View>
    <Text className="flex-1" style={{ fontSize: 14, fontWeight: '600', color: '#0F172A' }}>
      {label}
    </Text>
    {trailing || (
