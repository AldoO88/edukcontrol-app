// =====================================================================
// app/(social-worker)/exit-passes/index.jsx
// ---------------------------------------------------------------------
...
import GenerateExitPassModal from '@/app/(social-worker)/_components/GenerateExitPassModal';
...
// ---------------------------------------------------------------------
export default function SocialWorkerExitPassesScreen() {
  ...
  const renderExitPassCard = useCallback(({ item }) => {
    ...
    return (
      <Pressable
        onPress={() => router.push(`/(social-worker)/exit-passes/${item._id}`)}
  ...
  if (isLoading && items.length === 0) {
    return (
      <View className="flex-1 bg-slate-50">
        <DashboardHeader />
        <SchoolInfoCard school={school} isLoading={!school} className="mx-4 mt-4" teacher={dashboardData?.socialWorker} date={new Date().toISOString()} user={user} />
