// =====================================================================
// app/(director)/exit-passes/index.jsx
// ---------------------------------------------------------------------
...
import GenerateExitPassModal from '@/app/(director)/_components/GenerateExitPassModal';
...
// ---------------------------------------------------------------------
export default function DirectorExitPassesScreen() {
  ...
  const renderExitPassCard = useCallback(({ item }) => {
    ...
    return (
      <Pressable
        onPress={() => router.push(`/(director)/exit-passes/${item._id}`)}
  ...
  if (isLoading && items.length === 0) {
    return (
      <View className="flex-1 bg-slate-50">
        <DashboardHeader />
        <SchoolInfoCard school={school} isLoading={!school} className="mx-4 mt-4" teacher={dashboardData?.director} date={new Date().toISOString()} user={user} />
